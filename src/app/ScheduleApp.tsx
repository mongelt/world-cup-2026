...
import { LOCKED_SCORES } from "@/lib/schedule";
...
function statusTint(status: GameStatus): React.CSSProperties {
  if (status === "finished")
    return { background: "rgba(200,196,193,0.55)", opacity: 0.82 };
  if (status === "live")
    return {
      background: "rgba(107,42,42,0.13)",
      boxShadow: "0 0 0 2px rgba(107,42,42,0.35)",
    };
  return {};
}
...
export default function ScheduleApp({
  ...
  const setScore = useCallback(
    (matchNumber: number, side: "home" | "away", value: string) => {
  ...
  function resolveTeam(
    ...
    bracketResults: Map<
      number,
      { winner: StandingRow | null; loser: StandingRow | null }
    >,
  ): { code: string; iso: string; label: string } | null {
  ...
  function buildBracketResults(
    ...
    function getTeamForSeed(
      seed: string,
      matchRow: [number, string, string, string, string, string],
    ): StandingRow | null {
      if (seed === "3rd") {
        const wcMatches = R32_TOPOLOGY.filter(
          (r) => r[1] === "3rd" || r[2] === "3rd",
        );
    ...
    for (const row of ALL_TOPOLOGY) {
      ...
      if (hg > ag) results.set(matchNum, { winner: homeTeam, loser: awayTeam });
        ...
        results.set(matchNum, { winner: awayTeam, loser: homeTeam });
  ...
  function ScoreInputs({
    ...
    if (locked) {
      const lockedScore = LOCKED_SCORES[matchNumber] ?? { home: "", away: "" };
      return (
        <div
          style={{
            display: "flex",
        ...
        >
          <span>{lockedScore.home}</span>
          <span style={{ color: "var(--text-meta)", fontSize: "0.8rem" }}>
            –
          </span>
          <span>{lockedScore.away}</span>
        </div>
      );
    }
    ...
    return (
      <article
        style={{
          display: "grid",
          gridTemplateColumns:
            "96px minmax(220px,1.2fr) minmax(160px,1fr) minmax(140px,.8fr) minmax(120px,.7fr) auto",
          gap: 10,
          alignItems: "center",
  ...
  function TodayView() {
    ...
    const todayMatches = matches
      ...
      .sort(
        (a, b) =>
          (kickoffMinutesET(a.timeRaw) ?? 0) -
          (kickoffMinutesET(b.timeRaw) ?? 0),
      );
    const nextDay =
            ...
            <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
              <span
                style={{
                  ...
                  borderRadius: 3,
                  background: "rgba(107,42,42,0.13)",
                  border: "1px solid rgba(107,42,42,0.35)",
                  display: "inline-block",
                }}
              />
              Live
            ...
            <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
              <span
                style={{
                  ...
                  height: 12,
                  borderRadius: 3,
                  background: "rgba(200,196,193,0.55)",
                  border: "1px solid rgba(120,116,113,0.3)",
                  display: "inline-block",
                }}
              />
              Finished
        ...
        >
          ...
          {groupLetters.map((group) => (
            <React.Fragment key={group}>
              ...
              {chartDays.map((day) => {
                const dayMatches = groupStageMatches.filter(
                  (m) => m.group === group && m.date === day,
                );
                return (
                  <div
                    key={`${group}-${day}`}
  ...
  function ListView() {
    ...
    return (
      <HScrollList>
        {filtered.map((m) => (
          <MatchCard key={m.matchNumber} match={m} showDate />
        ))}
      </HScrollList>
    );
  }
  ...
  function StatusPill({ status }: { status: TeamStatus }) {
    ...
    > = {
      advanced: {
        label: "✓ Advanced",
        bg: "rgba(43,107,50,0.12)",
        color: "#2b6b32",
        border: "rgba(43,107,50,0.35)",
      },
      eliminated: {
        label: "✗ Eliminated",
        bg: "rgba(120,116,113,0.14)",
        color: "var(--text-meta)",
        border: "rgba(120,116,113,0.28)",
      },
      contention: {
        label: "In contention",
        bg: "rgba(107,42,42,0.08)",
        color: "var(--accent)",
        border: "rgba(107,42,42,0.22)",
      },
      pending: {
        label: "Not started",
        bg: "rgba(180,170,165,0.14)",
        color: "var(--text-meta)",
        border: "rgba(120,116,113,0.2)",
      },
    };
    const c = cfg[status];
    return (
                    ...
                    >
                      <td
                        style={{
                          ...
                          fontWeight: 800,
                          fontSize: "0.88rem",
                          color: idx < 2 ? "var(--accent)" : "var(--text-meta)",
                        }}
                      >
                        {idx + 1}
                      </td>
      ...
      <div>
        ...
        >
          ...
          >
            ...
            from the 12 third-place finishers. Rankings update live as scores
            are entered.
            {thirdPlaceTeams.length < totalGroups &&
              playedCount < totalGroups && (
                <span style={{ color: "var(--text-meta)" }}>
                  {" "}
                  ·{" "}
                  {totalGroups - thirdPlaceTeams.length > 0
                    ? `${totalGroups - thirdPlaceTeams.length} groups not yet started`
                    : `${totalGroups - playedCount} groups have no scores yet`}
                </span>
              )}
          </div>
        </div>
        ...
        >
          <div
            style={{
              ...
              borderRadius: 18,
              overflow: "hidden",
              minWidth: WILDCARD_TABLE_MIN_WIDTH,
            }}
          >
              ...
              <tbody>
                {thirdPlaceTeams.length === 0 ? (
                  <tr>
                    ...
                    >
                      ⚽ Waiting for group stage scores — enter results in
                      Standings or Chart view.
                    </td>
                  </tr>
                ) : (
                  thirdPlaceTeams.map((team, idx) => {
                    ...
                    return (
                      <React.Fragment key={team.code}>
                        {idx === advanceCount &&
                          thirdPlaceTeams.length > advanceCount && (
                            <tr key="cutline">
                              <td
                                colSpan={thHeaders.length}
                                style={{
  ...
  function BracketView() {
    ...
    const wildcardSlots: (StandingRow | null)[] = Array.from(
      { length: 8 },
      (_, i) => thirdPlaceTeams[i] ?? null,
    );
          ...
          >
            {wc ? (
              <>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={`https://flagcdn.com/w40/${wc.iso}.png`}
                  alt={wc.name}
                  width={18}
                  height={13}
                  loading="lazy"
        ...
        >
          {hasTeam ? (
            <>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={`https://flagcdn.com/w40/${resolved!.iso}.png`}
                alt={resolved!.code}
                width={18}
                height={13}
                loading="lazy"
    ...
    function BracketCard({ matchNum }: { matchNum: number }) {
      ...
      return (
        ...
        >
          <div
            style={{
              background: "rgba(215,205,202,0.5)",
              padding: "3px 8px",
              display: "flex",
              justifyContent: "space-between",
              alignItems: "center",
          ...
          >
            ...
            >
              {shortDate(date)} · {parseTime(timeRaw)}
            </span>
          </div>
          <div
            style={{
              ...
              alignItems: "center",
              justifyContent: "space-between",
              borderBottom: "1px solid rgba(46,42,40,0.07)",
            }}
          >
          ...
          <div
            style={{
              padding: "3px 8px 4px",
              background: "rgba(215,205,202,0.2)",
              borderTop: "1px solid rgba(46,42,40,0.07)",
            }}
          >
  ...
  return (
    ...
    >
            ...
            {(
              ...
              <button
                ...
                style={{
                  ...
                  color: view === v ? "#fff" : "rgba(240,236,232,0.6)",
                  fontFamily: "var(--font-ui)",
                  fontWeight: 700,
                  fontSize: "0.78rem",
      ...
      <main
        style={{ maxWidth: 1400, margin: "0 auto", padding: "20px 16px 48px" }}
      >
